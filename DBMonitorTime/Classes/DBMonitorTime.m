...
- (void)startMonitoringTimer{
    ...
    dispatch_source_set_event_handler(self.monitoringTimer, ^{
        ...
        NSDictionary * mainThreadCallStack = [DBBackTraceLogger dbBacktraceMapOfMainThread];
        for (NSString * funcAddress in mainThreadCallStack.allKeys) {
            NSString * funcName = [mainThreadCallStack objectForKey:funcAddress];
            DBMonitorTimeModel * model = [self.callStackDict objectForKey:funcAddress];
            if (model == nil){
                model = [DBMonitorTimeModel new];
...
- (void)logAllCallStack{
    ...
    for (NSString * key in self.callStackDict.allKeys) {
        DBMonitorTimeModel * model = [self.callStackDict objectForKey:key];
...
@implementation DBMonitorTimeModel
